...
const Home = () => {
    ...
    //소켓 연결 확인
    useEffect(() => {
        const socket = io('http://localhost:8081'); // 서버 주소를 사용하세요.
        ...
        socket.on('serverMessage', (data) => {
          // 서버로부터 메시지를 받았을 때의 처리 기능
        });
    
        // 컴포넌트가 언마운트되면 소켓 연결을 해제한다.
        return () => {
          socket.disconnect();
        };
      }, []);
    ...
    const onSendMessage = (text) => {
        ...
        // 추가로 실행할 동작이 필요한 경우, 여기 작성하세요.
        // 예시: 상대방(TYPE: partner)의 메시지를 받아오고 렌더링하는 것 등
      };
        ...
        <div>
            <Suspense fallback={<Fallback />}>
                <div className={"fullScreen"}>
                        ...
                        <ul>
                            ...
                            {loggedIn ? (
                                ...
                                <>
                                    ...
                                    <li className="menu_li">
                                    <a className="menu_a">V.chat</a>
                                     <ul className="menu_li">
                                     <li style={{ fontSize: '24px', margin: '10px' }} onClick={handleShowDrag}>
                                      랜덤채팅
                                    </li>
                                    </ul>
                                    </li>
                   ...
                   {/* 드래그 채팅창 사이드 밖 영역 */}
                    <Drag show={showDrag} onClose={handleDragClose}>
                    <ChatBox messages={messages} 
                    onSendMessage={onSendMessage} chatMessagesRef={chatMessagesRef} onCloseChat={handleCloseChat} />
                    </Drag>
                </div>
            </Suspense>
        </div>
