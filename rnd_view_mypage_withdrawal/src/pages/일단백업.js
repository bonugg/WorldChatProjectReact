...
const RandomChatDrag = React.memo(({ show, onClose, logoutApiCate }) => {
    ...
    const onDragStop = (e,d) => {
        let newPosition={
            x:d.x < 0 ? 0 : d.x + parseInt(size.width) > window.innerWidth ? window.innerWidth - parseInt(size.width) : d.x,
            y:d.y < 0 ? 0 : d.y + parseInt(size.height) > window.innerHeight ? window.innerHeight - parseInt(size.height) : d.y
        }
    
        setPosition(newPosition);
     };

     const onResizeStop =(e,directions,x,y,delta,width,height)=>{
        let newSize={
            width:`${width}px`,
            height:`${height}px`
        }
        ...
        let newPosition={
            x:(position.x + delta.width) > window.innerWidth ? position.x-delta.width : position.x,
            y:(position.y + delta.height) > window.innerHeight ? position.y-delta.height : position.y
        }
