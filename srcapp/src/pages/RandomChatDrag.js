...
const RandomChatDrag = React.memo(({ show, onClose, logoutApiCate }) => {
    ...
    const [hasMore, setHasMore] = useState(true); //더 많은 채팅 메시지가 있는지
    const [messageGroups, setMessageGroups] = useState([]); //메시지 전체를 담는 변수
    const [messageGroupsItem, setMessageGroupsItem] = useState([]); //메시지 100개를 담는 변수
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            if (scrollElement.scrollTop <= 0 && currentGroupIndex > 0) {
                //스크롤이 맨 위에 있을 때
                // 현재 보이는 화면의 하단에서부터 약 10% 상단에 위치
                scrollElement.scrollTop = scrollElement.clientHeight * 0.95;
            }
            else if (scrollElement.scrollHeight - scrollElement.scrollTop === scrollElement.clientHeight
                && currentGroupIndex < messageGroups.length) {
                //스크롤이 맨 아래에 있을 때
                // 현재 보이는 화면의 상단에서부터 약 10% 아래에 위치
                scrollElement.scrollTop = (scrollElement.scrollHeight - scrollElement.clientHeight) * 0.05;
            }

        };

        // if (currentGroupIndex > messageGroups.length) {
        //     setCurrentGroupIndex(messageGroups.length);
        // }
    ...
    //-------------------무한스크롤 감시 및 처리 IntersectionObserver 설정-----------------------------
    useEffect(() => {
        const observer = new IntersectionObserver(
            (entries) => {
                console.log("01010101010101010101");
                if (entries[0].isIntersecting && hasMore && !isLoading) {
                    //스크롤이 바닥에 닿았을 때 더 많은 메시지 로딩
                    console.log("01010101010101010101");
                    setIsLoading(true);
                    ...
                    loadMoreMessages()
                        ...
                        .catch((error) => {
                            console.error("Error load more messages: ", error);
                            setIsLoading(false);
                        });
                }
            },
    ...
    //-------------------무한스크롤 시 새로운 메시지를 가져오는 비동기 요청-----------------------------
    const loadMoreMessages = async (content) => {
        try {
            //로딩 중 상태 true로 설정
            setIsLoading(true);

            //가져온 메시지를 messageGroupsItem에 추가
            setMessageGroupsItem((prev) => [...prev, content]);

            //로드할 메시지 없으면 hasMore를 false로 설정
            if (messages.length === 0) {
                setHasMore(false);
            } else {
                setMessageGroups((prev) => [...prev, messageGroupsItem]);
                setMessageGroupsItem([]);
            }
        } catch (error) {
            console.error("Error loading more messages: ", error);
        } finally {
            //로딩된 상태를 다시 false로 설정
            setIsLoading(false);
        }
    }
    useEffect(() => {
        console.log("messageGroupsItem 인덱스: ", messageGroupsItem.length);
        if (messageGroupsItem.length === 10) {
            setMessageGroups((prev) => [...prev, messageGroupsItem.slice()]);
            setMessageGroupsItem([]);
        }
    }, [messageGroupsItem])

    useEffect(() => {
        console.log("messageGroups.length: ", messageGroups.length);
        //setCurrentGroupIndex(messageGroups.length);
    }, [messageGroups]);
    ...
    useEffect(() => {
        ...
        if (scrollElement) {
            ...
            if (!previousScrollbarStateRef.current && hasScrollbar) {
                // 스크롤바가 처음 생겼을 때, 전체 높이의 20% 아래로 위치시킵니다.
                scrollElement.scrollTop = scrollElement.scrollHeight * 0.05;
            } else if (isScrollRef.current) {
                // 현재 보이는 화면 중간으로 스크롤바 위치 설정
                scrollElement.scrollTop = (scrollElement.scrollHeight - scrollElement.clientHeight) / 2;
            } else if (!isScrollRef.current) {
                // 현재 보이는 화면 하단에서 20% 위로 스크롤바 위치 설정
                scrollElement.scrollTop = (scrollElement.scrollHeight - scrollElement.clientHeight) * 0.95;
            }
    ...
    const isScrollbarAtBottom = (element) => {
        ...
        const scrollThreshold = 1; // 스크롤 바가 바닥에 있는 것으로 판단할 수 있는 임계 값
        const scrollHeight = element.scrollHeight; //goek
        return element.scrollHeight - element.scrollTop - element.clientHeight <= scrollThreshold;

    };

    //-------------스크롤 위치에 따른 MessageGroups
    const handleTopMessagesLoad = () => {
        //이전 메시지를 보기 위해 스크롤을 올린 경우
        if (currentGroupIndex > 0) {
            //메시지들을 모아놓은 MessageGroups의 인덱스를 내려 이전 페이지를 보여준다.
            setCurrentGroupIndex((prevIndex) => prevIndex - 1);
        }
    };

    const handleBottonMessagesLoad = () => {
        //다음 메시지를 보기 위해 스크롤을 내린 경우
        if (currentGroupIndex < messageGroups.length) {
            //메시지들을 모아놓은 MessageGroups의 인덱스를 높여 다음 페이지를 보여준다.
            setCurrentGroupIndex((prevIndex) => prevIndex + 1);
        }
        else {
            setCurrentGroupIndex(messageGroups.length);
    ...
    const isScrollbarNearTop = element => element.scrollTop <= scrollThreshold;
    const isScrollbarNearBottom = element => element.scrollHeight - element.scrollTop - element.clientHeight <= scrollThreshold;

    const handleScrollMessage = (event) => {
        const element = event.target;
        setScroll(element.scrollTop);

        if (isScrollbarNearTop(element)) {
            handleTopMessagesLoad();
            setIsScroll(false);
        }
        else if (isScrollbarNearBottom(element)) {
            handleBottonMessagesLoad();
            setIsScroll(true);
        }
    };
                                    ...
                                    <div className={"EnterRoomChat"}>
                                        <div className={"EnterRoomChat_2"}>
                                            <div className={"EnterRoomChat_content"}>
                                                <div className={"EnterRoomChat_content_2"} onScroll={handleScrollMessage}>
                                                    {(currentGroupIndex > -1 && currentGroupIndex < messageGroups.length) ?
                                                        (
                                                            messageGroups[currentGroupIndex] && messageGroups[currentGroupIndex].map((message, index) => {
                                                            ...
                                                            messageGroupsItem.map((message, index) => {
                                                                const isMyMessage = message.sender === LoginUserNickName.current;
                                                                return (
                                                                    <MessageStyled
                                                                        key={index}
